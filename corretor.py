...
def procuraPalavra(palavra):
    resultado = 1 #1achou e 0 nao achou
    menordist = 999999999999
    sugestoes = []
    arq = open('ptbr.txt')
    while True:
        lineorig = arq.readline()
	line = lineorig
        linePronta = removeBarra(line)
        linePronta = linePronta.lower()
        palavraPronta = palavra.lower()
	if linePronta.isalpha():
	    if palavraPronta == linePronta:
		sugestoes = []
		break
	    else:
		resultado = 0
		calc = len(linePronta) - len(palavraPronta)
		if calc < 0:
		    calc = calc * -1
		if menordist >= calc:
		    dist =  stredit2(linePronta,palavraPronta)
		    if dist == menordist:
			sugestoes.append(linePronta)
		    if dist < menordist:
			menordist = dist
			sugestoes = [linePronta]
	    

        if len(lineorig) == 0: # Zero indica fim de arquivo (EOF)
            break
...
def execute(nomearq):
    ...
    texto = f.read()
    space = " "
    lista = texto.split(space)

    for word in lista:
        if word.isalpha():
	    resultado = procuraPalavra(word)
	    
	    if resultado != [] :
		resultado.append("manter a palavra")
		print "palavra ",word," nao encontrada \n"
		count = 0
		for op in resultado:
		    print "digite ",count," para trocar por ",op," \n"
		    count = count + 1
		opcao = input("qual opcao?")
		if opcao+1 != count:
		    texto = texto.replace(word,resultado[opcao])
    print texto
    f.close()
    a = open(nomearq,'w')
    a.write(texto)
...
def removeBarra(remover):
    aux = remover.find("/")
    if aux != -1:
        removido = remover[:aux]
    else: 
        removido = remover
    return removido    
...
def stredit2 (s1,s2, showtable=False):
